...
# the probe constant is the brief's
brief = pathlib.Path('TASK.md').read_text()
block = re.search(r'## 4\. `PROBE`.*?```python\n(.*?)```', brief, re.S).group(1)
assert m.PROBE.strip() == block.strip(), 'PROBE is not the brief constant'
...
assert all(w in doc for w in ('projection.json', 'otillgänglig', 'schemalagd')) and len(doc) > 1500
