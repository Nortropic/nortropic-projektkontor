...
`ValueError` (without private values in the message) for invalid input; a `paused` that is missing or not a boolean is
not invalid input (rule 4). Input `readings` (all keys required):
...
4. Utkiken: `schedule` is `stoppat` when `note` is a string starting with `STOPPED`, else `pausat` when `paused` is true,
   `aktiverat` when `paused` is false, and `okänt` otherwise (the watch view's own rule): a `paused` that is missing or
   not a boolean (for example `"ja"`, `1` or null) gives `okänt` and never an error, and the rest of Utkiken is still
   read. `next_planned` is the earliest
...
`tools/aquarium.py`, in more than 1 500 characters.

How the host verifies, as measured: it copies only the three files above into a fresh clone of the repository at the
base (no `TASK.md` and nothing else uncommitted is there) and runs its frozen acceptance in that clone inside a sandbox.
There the repository is read-only except the existing `.scratch` directory: files and directories may be created and
...
with `-I -B`. No sibling `Nortropic Runtime` exists, so a real `collect` finds every Runtime source unavailable, which
is not a failure. The host audits `project` itself for any file, process, socket, `os` or `ctypes` event: inside
`project` nothing is imported, opened, read or run, so the modules `project` uses are imported at module level. This
does not change section 2: the existing `bevakningsbild` is still imported only inside the default `watch_reader`. The
host runs `python3 -I -B -m unittest discover -s tools -p test_aquarium.py`, which must pass and leave `.scratch`
exactly as it found it, and `python3 -I -B tools/aquarium.py` with a new directory under `.scratch`.
