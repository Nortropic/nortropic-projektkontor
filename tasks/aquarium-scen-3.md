...
You work with file tools only and cannot run the tests, so make every assertion follow from a rule of this brief and check the
existing tests by reading them. At the base, these existing tests in `tools/test_aquarium.py` touch facts that schema 2
changes (the schema number, the eight sources, `verkstaden`, the `sockeln` and headline counts, engine executions, the
collector): `Envelope.test_sources_titles_and_staleness`, `Envelope.test_output_keys_are_exactly_the_contract`,
`Envelope.test_input_is_never_mutated`, `Envelope.test_invalid_input_is_refused_without_private_values`,
`UnavailableSources.test_office_unavailable_beside_working_sources`, `UnavailableSources.test_engine_unavailable`,
`Verkstaden.test_identity_idle_busy_and_workflow_task`, `Verkstaden.test_review_activity_is_under_review`,
`Headline.test_calm_only_when_everything_is_read_and_empty`, `Headline.test_waiting_counts_the_watch_and_the_questions`,
`Headline.test_unknown_is_never_zero`, `DisplaySafety.test_unlisted_input_keys_are_never_copied`,
`Collect.test_a_full_reading_projects`, `Collect.test_a_failing_probe_leaves_only_runtime_parts_unavailable`,
`Collect.test_a_malformed_part_is_only_its_own_source` and `Command.test_unavailable_sources_are_not_a_failure`, with
the helpers `values()`, `readings()` and `execution()` that build their inputs (an engine execution now carries `id`).
Check each against schema 2 and adjust every assertion
that schema 2 makes false (for example `Collect.test_a_full_reading_projects` asserts `result['schema'] == 1`), keeping
each test's purpose. The host runs every test in both test files, and a single assertion left on schema 1 fails the task.
